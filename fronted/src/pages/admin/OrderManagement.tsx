...
const OrderManagement: React.FC = () => {
  ...
  useEffect(() => {
    apiGet('/admin/orders').then(data => setOrders(data)).catch(() => {});
  }, []);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-text-muted mt-1">查看和管理所有用户订单</p>
        </div>
      ...
      <div className="bg-bg-card rounded-xl border border-accent overflow-hidden">
          ...
          <table className="min-w-full divide-y divide-accent">
            <thead>
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">订单号</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">用户</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">商品</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">金额</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-text-muted uppercase tracking-wider">支付方式</th>
        ...
        {filteredOrders.length === 0 && (
          <div className="p-12 text-center">
            ...
            <p className="text-text-muted">当前没有符合条件的订单</p>
          </div>
        )}
